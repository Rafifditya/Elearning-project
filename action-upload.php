...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], 'files/'.$_FILES['file']['name'])) {
		...
		$sql = "UPDATE datamhs SET nama_File = '$nama_file' WHERE nrp = '$nrp'";
    $path_file = 'files/'.$nama_file;

		if(mysqli_query($conn, $sql)){
			echo "Upload Berhasil!";
		$sql2 = "INSERT INTO file(file,nrp,path_file,type_file,size_file) VALUES ('$nama_file','$nrp','$path_file','$filetype','$filesize')";
