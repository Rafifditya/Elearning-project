...
//Ambil data
if ($cek == 1) {
	$cariMhs = mysqli_query($conn, "SELECT * FROM dosen WHERE nrp='$nrp'");
	$dataMhs = mysqli_fetch_array($cariMhs);
} else {
	$cariMhs = mysqli_query($conn, "SELECT *FROM datamhs WHERE nrp='$nrp'");
	$dataMhs = mysqli_fetch_array($cariMhs);
}
...
<head>
  ...
  <title>E - Leaps</title>
    ...
    <body>
      ...
      $sql = "SELECT * FROM datamhs WHERE nrp = ".$_POST['nrp'];
      $result = mysqli_query($conn, $sql);

			$cari = mysqli_query($conn, "SELECT *FROM massage WHERE nrp='$nrp'");
			$msg = mysqli_fetch_array($cari);
			$cari2 = mysqli_query($conn, "SELECT *FROM massage");
      if (mysqli_num_rows($result) > 0) {
        ?>
		      ...
		      <p>anda di minta dosen anda segera mengirim kan nya terimakasih.</p>
					<?php }
						}
					}
						?>
		                ...
		                <td>
											...
											<?php if ($dataMhs['role_id'] == 2) { ?>
												<?php if ($row2['nrp'] == $row["nrp"]) { ?>
													<a href="#" class="button input"><i class="fa fa-flash" aria-hidden="true"></i>Sedang diproses upload</a>
																<?php } else { ?>
																	<a href="action-alert.php?nrp=<?=$row["nrp"];?>" class="button input"><i class="fa fa-flash" aria-hidden="true"></i>Meminta Segera Mengirim E-learning</a>
																<?php } ?>
											<?php } ?>
											<?php if ($dataMhs['role_id'] == 1) {?>
